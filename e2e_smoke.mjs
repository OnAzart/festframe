...
const wallpaperSafeArea = await page.locator('.iphone-export.is-previewing .iphone-card').evaluate((card) => {
  ...
  return {
    ...
    artistFontSizes: [...new Set(sets.map((set) => getComputedStyle(set.querySelector('strong')).fontSize))],
    legendAlignedLeft: getComputedStyle(card.querySelector('.wallpaper-priority-legend')).justifyContent === 'flex-start',
...
if (wallpaperSafeArea.minimumSetHeight < 36 || wallpaperSafeArea.artistFontSizes.join(',') !== '11px' || !wallpaperSafeArea.legendAlignedLeft) throw new Error('Dense wallpaper typography or legend regressed')
